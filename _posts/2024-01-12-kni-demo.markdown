...
```
git clone https://github.com/MikeZappa87/kubernetes.git
cd kubernetes/
...
Build the CNI plugins because we'll use the `bridge` CNI plugin which flannel uses internally.
...
*NB*: [I have a modified setup.sh](https://github.com/dougbtv/kni-demo/blob/flannel-bridge/setup.sh#L17) which installs the bridge CNI plugin (via `docker cp`).

```
./setup.sh
```

After that, we'll install flannel as our default CNI plugin.
